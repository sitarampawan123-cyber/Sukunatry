...
def generate_random_name():
    ...
    name = 'Taitan'+''.join(random.choice(characters) for _ in range(6)).upper()
    return name
...
def save_account_to_json(region, uid, password):
    """Save account to JSON file based on region"""
    filename = f"{region.lower()}_id.json"
    account_data = {
        "uid": uid,
        "password": password
    }
...
#os.system('title GUEST GENERATOR v1 By Taitan ^| @mohp4')

print(f"""\n\n\n{Fore.RED+bold}\n                                           {Fore.RED}@{Fore.RESET}TaitanKING\n""")
...
for i in range(50):
    try:
        ...
        if status_code == 200:
            # Save to JSON file
            save_account_to_json(region, uid, password)
...
print(f"{lg}{bold}Finished! Accounts saved to {region.lower()}_id.json")